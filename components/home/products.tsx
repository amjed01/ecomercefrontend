...
// Use a base64 encoded placeholder to avoid 404 errors
...
const formatImageUrl = (product: Product): string => {
  ...
  const fetchProducts = async () => {
    ...
    setError("");
    try {
      const response = await axios.get<ApiResponse>(`${API_BASE_URL}/products`);
      ...
      if (response.data.success && response.data.products) {
        ...
        response.data.products.forEach((product, index) => {
          console.log(`Product ${index}:`, {
            ...
            formattedUrl: formatImageUrl(product)
          });
        });
      } else {
        setData([]);
  ...
  const handleImageError = (e: React.SyntheticEvent<HTMLImageElement, Event>, productId: string) => {
    console.log("Image failed to load for product:", productId, "src:", e.currentTarget.src);
    ...
    <div className="py-8 px-4 sm:px-6 lg:px-8">
      <div className="max-w-7xl mx-auto">
        ...
        {data.length === 0 ? (
          ...
          <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-3 lg:grid-cols-5 xl:grid-cols-5 gap-6">
            {data.map((product) => {
              ...
              return (
                ...
                >
                  ...
                  <Link href={`/productdetail/${product._id}`} className="block">
                   <div className="h-40 mb-4 flex items-center justify-center overflow-hidden rounded-xl bg-gray-50" style={{ minHeight: 160 }}>
  <img
    src={imageUrl}
    alt={product.title}
